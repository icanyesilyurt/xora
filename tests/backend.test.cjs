...
test('strict request validation before billing',()=>{
 ...
 for(const patch of [{mode:'fun'},{mode:'oops'},{locale:'fr'},{locale:'pt'},{locale:'ar'},{locale:'ES'},{locale:'es-ES'},{handle:'abc!'},{handle:'a b'},{handle:'a'.repeat(16)},{handle:32},{request_id:'../id'},{mode:'match',handles:['Alice','@alice']}]) assert.throws(()=>e.validateRequest({...base,...patch}),/bad_request/);
});
...
test('REAL AI output is generated for the request locale only (tr, en, es)',()=>{
 ...
 for(const locale of ['tr','en','es']){
  ...
  for(const mutation of [r=>delete r.comment,r=>r.comment='',r=>r.tagline='Se analizaron 25 publicaciones.',r=>delete r.summary,r=>delete r.metrics[0].label]){
...
test('REAL schema asks for one set of user-facing strings, independent of locale count',()=>{
 ...
 assert.deepEqual(Object.keys(e.REAL_LOCALES),['tr','en','es']);
});
