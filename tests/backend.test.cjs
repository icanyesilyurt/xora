...
test('strict request validation before billing',()=>{
 ...
 for(const locale of ['tr','en','es','pt','ar','fr']) assert.equal(e.validateRequest({...base,locale}).locale,locale);
 // Planned but not yet served locales are rejected before billing until they have REAL language rules.
 for(const patch of [{mode:'fun'},{mode:'oops'},{locale:'ja'},{locale:'it'},{locale:'de'},{locale:'FR'},{locale:'fr-FR'},{locale:'ES'},{locale:'es-ES'},{locale:'PT'},{locale:'pt-BR'},{locale:'AR'},{locale:'ar-SA'},{handle:'abc!'},{handle:'a b'},{handle:'a'.repeat(16)},{handle:32},{request_id:'../id'},{mode:'match',handles:['Alice','@alice']}]) assert.throws(()=>e.validateRequest({...base,...patch}),/bad_request/);
});
...
test('REAL AI output is generated for the request locale only (tr, en, es, pt, ar, fr)',()=>{
 const e=edge();const sig={question_ratio:.8,own_posts:8};
 const otherKeys=/(?:_tr|_en|_es)$/;
 for(const locale of ['tr','en','es','pt','ar','fr']){
  ...
  for(const mutation of [r=>delete r.comment,r=>r.comment='',r=>r.tagline='Se analizaron 25 publicaciones.',r=>r.summary='Foram analisadas 25 publicações.',r=>r.comment='تم تحليل 25 منشورًا.',r=>r.tagline='حللنا ٢٥ تغريدة.',r=>r.summary='Après 25 publications, ce compte pose beaucoup de questions.',r=>r.comment='Publications analysées : 25 au total.',r=>delete r.summary,r=>delete r.metrics[0].label]){
...
test('REAL schema asks for one set of user-facing strings, independent of locale count',()=>{
 ...
 assert.deepEqual(Object.keys(e.REAL_LOCALES),['tr','en','es','pt','ar','fr']);
});
