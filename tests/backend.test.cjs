...
test('strict request validation before billing',()=>{
 ...
 for(const locale of ['tr','en','es','pt','ar','fr','de','it','ja','ko']) assert.equal(e.validateRequest({...base,locale}).locale,locale);
...
test('REAL AI output is generated for the request locale only (tr, en, es, pt, ar, fr, de, it, ja)',()=>{
 ...
 for(const locale of ['tr','en','es','pt','ar','fr','de','it','ja','ko']){
...
test('REAL schema asks for one set of user-facing strings, independent of locale count',()=>{
 ...
 assert.deepEqual(Object.keys(e.REAL_LOCALES),['tr','en','es','pt','ar','fr','de','it','ja','ko']);
});
