...
test('REAL requests produce copy only for their own locale (tr, en, es, pt) through both providers',async()=>{
 for(const provider of ['openai','anthropic'])for(const locale of ['tr','en','es','pt'])for(const mode of ['mirror','stalk','match']){
 ...
 for(const locale of ['fr','it','ar','xx','pt-BR']){
