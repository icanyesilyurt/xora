...
test('canonical FUN personas cover every supported locale with reviewed independent copy',()=>{
 ...
 assert.deepEqual(locales,['ar','de','en','es','fr','it','pt','tr']);
...
test('nickname guard rejects forced compounds and sensitive labels in every locale',()=>{
 ...
 for(const lang of ['tr','en','es','pt','ar','fr','de','it'])for(const text of bad[lang])assert.equal(e.aliasValid(text,lang),false,lang+': '+text);
...
test('REAL fallback is safe, deterministic and signal based without another AI call',()=>{
 ...
 const unnatural={tr:'Soru Makinesi',en:'Question Machine',es:'Máquina de Preguntas',pt:'Máquina de Perguntas',ar:'آلة الأسئلة',fr:'Machine à Questions',de:'Die Fragemaschine',it:'Macchina delle Domande'};
 for(const signals of cases)for(const locale of ['tr','en','es','pt','ar','fr','de','it']){
