...
test('canonical FUN personas cover every supported locale with reviewed independent copy',()=>{
 ...
 for(const p of c.FUN_PERSONAS){
  ...
  assert.equal(c.funModeComment(card,'mirror','es'),'','no translated or cross-language copy invented');
 }
...
test('nickname guard rejects forced compounds and sensitive labels in both locales',()=>{
 const e=edge();
 const bad={
 ...
 for(const lang of ['tr','en'])for(const text of bad[lang])assert.equal(e.aliasValid(text,lang),false,lang+': '+text);
 for(const [lang,text] of [['tr','Meraklı Muhabbetçi'],['en','Thoughtful Conversationalist'],['tr','Koltuk Filozofu'],['en','Armchair Philosopher'],['en','Social Butterfly'],['en','INQUISITIVE MIND']]){
  assert.equal(e.aliasValid(text,lang),true,text);
 }
 assert.equal(e.aliasValid('Curious Mind','es'),false);
...
test('REAL fallback is safe, deterministic and signal based without another AI call',()=>{
 ...
 for(const signals of cases){
  ...
  const b=e.pickAliasPair({nickname_candidates:[{tr:'Soru Makinesi',en:'Question Machine',evidence:'question_ratio'}]},signals);
  assert.equal(a.source,'fallback');assert.equal(JSON.stringify(a),JSON.stringify(b));
  for(const lang of ['tr','en'])assert.equal(e.aliasValid(a[lang],lang),true,a[lang]);
 }
 const signals={question_ratio:.7};
 for(const candidate of [{tr:'Meraklı Biri',en:'Question Machine'},{tr:'Soru Makinesi',en:'Curious Mind'}]){
  assert.equal(e.pickAliasPair({nickname_candidates:[{...candidate,evidence:'question_ratio'}]},signals).source,'fallback');
 }
 assert.equal(e.pickAliasPair({nickname_candidates:[{tr:'Meraklı Muhabbetçi',en:'Thoughtful Conversationalist',evidence:'question_ratio'}]},signals).source,'ai_generated_validated');
});
