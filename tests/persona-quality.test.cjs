...
test('canonical FUN personas cover every supported locale with reviewed independent copy',()=>{
 ...
 assert.deepEqual(locales,['ar','de','en','es','fr','pt','tr']);
 ...
 for(const p of c.FUN_PERSONAS){
  ...
  // Spanish, Portuguese, French and German copy must not be an English word-mix.
  for(const lang of ['es','pt','fr','de']) assert.doesNotMatch(p.locales[lang].nickname,/\b(?:the|of|and|you|room|mind|quiet|update)\b/i,lang+' '+p.id);
...
test('REAL fallback is safe, deterministic and signal based without another AI call',()=>{
 ...
 const unnatural={tr:'Soru Makinesi',en:'Question Machine',es:'Máquina de Preguntas',pt:'Máquina de Perguntas',ar:'آلة الأسئلة',fr:'Machine à Questions',de:'Die Fragemaschine'};
 for(const signals of cases)for(const locale of ['tr','en','es','pt','ar','fr','de']){
